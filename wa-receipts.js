...
const paintBrand=()=>{
 document.title=document.title.replaceAll('WhatsAfrica','WASSAFRICA');
 document.body.innerHTML=document.body.innerHTML.replaceAll('WhatsAfrica','WASSAFRICA');
};
const enterFix=()=>{
...
const restartIfAuthenticated=async()=>{
 ...
 for(let i=0;i<10&&!session;i++){
   try{session=(await client.auth.getSession()).data?.session||null}catch(e){}
   if(!session&&i<9)await wait(300);
 }
 if(!session)return;
 ...
 if(inline){
   try{
     ...
     await wait(400);
   }catch(error){console.error('WASSAFRICA inbox recovery failed',error)}
 }
...
window.addEventListener('error',event=>{
 ...
 if(/Unexpected token|Unexpected identifier|boot|supabase/i.test(msg))setTimeout(restartIfAuthenticated,0);
});
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>setTimeout(restartIfAuthenticated,250),{once:true});
else setTimeout(restartIfAuthenticated,250);
setTimeout(restartIfAuthenticated,1800);
setTimeout(restartIfAuthenticated,4000);
})();