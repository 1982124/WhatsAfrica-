...
async function share(data={}){if(navigator.share){try{await navigator.share({title:data.title||'WASSAFRICA',text:data.text||'',url:data.url||location.href,files:data.files||undefined});return}catch(e){if(e?.name==='AbortError')return}}try{await navigator.clipboard.writeText(data.url||location.href);alert('Lien copié. Vous pouvez le coller dans le réseau de votre choix.')}catch{window.open(data.url||location.href,'_blank','noopener,noreferrer')}}
...
window.WA_EXTERNAL_CONNECT={version:1,open,close,share,call,networks:NETWORKS};
function wire(){install();const tools=document.querySelector('.tools');if(!tools||tools.dataset.waExt)return;tools.dataset.waExt='1';const b=document.createElement('button');b.id='waExternalBtn';b.className='tool';b.type='button';b.textContent='🌐 Réseaux';b.title='Ouvrir un réseau ou partager';b.onclick=e=>{e.preventDefault();open()};tools.appendChild(b)}
