 ...
 * Repairs metadata for legacy media rows without breaking the message
 * content constraint: encryption_version=1 is retained when media_path is
 * encrypted storage, while the client is told that ciphertext is absent.
 */
(()=>{
'use strict';
...
async function repair(){if(running)return;const s=sess();if(!s)return;running=true;try{const mem=await rest('conversation_members','select=conversation_id&user_id=eq.'+encodeURIComponent(s.user.id));const ids=[...new Set((mem||[]).map(x=>x.conversation_id))];for(const cid of ids){const rows=await rest('messages_v2','select=id,encryption_metadata,message_type,media_path&conversation_id=eq.'+encodeURIComponent(cid)+'&sender_id=eq.'+encodeURIComponent(s.user.id)+'&message_type=in.(image,video,audio,file,voice)&encryption_version=eq.1&ciphertext=is.null&limit=100');for(const r of rows||[]){const meta={...(r.encryption_metadata||{}),transport:r.encryption_metadata?.transport||'legacy-private-storage',ciphertext_present:false,media_recovery:'key_recovery_required',consistency_checked_at:new Date().toISOString()};await rest('messages_v2','id=eq.'+encodeURIComponent(r.id)+'&sender_id=eq.'+encodeURIComponent(s.user.id),{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({encryption_metadata:meta})})}}}catch{}finally{running=false}}
