...
const sanitize=a=>(Array.isArray(a)?a:[]).slice(-50).map(x=>({id:x.id,conversationId:x.conversationId,ciphertext:x.ciphertext,metadata:x.metadata,savedAt:x.savedAt||Date.now()}));
const read=()=>{try{return sanitize(JSON.parse(localStorage.getItem(key()||'')||'[]'))}catch{return[]}};
const write=a=>{try{localStorage.setItem(key(),JSON.stringify(sanitize(a)))}catch{}};
let running=false;
