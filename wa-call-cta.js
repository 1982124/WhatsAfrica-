(function(){'use strict';
/* WhatsAfrica: CTA appels + stabilisateur de session Cockpit.
   Le Cockpit utilise Supabase Auth comme source de vérité et ce module
   ré-affiche le workspace dès qu'une session authentifiée est détectée. */
function stabilizeCockpitAuth(){
  ...
  const showLoggedIn=async(session)=>{
    ...
    try{
      ...
      if(typeof enter==='function') await enter(session.user);
      if(auth)auth.classList.add('hidden');
      if(app)app.classList.remove('hidden');
      if(logout)logout.classList.remove('hidden');
      if(msg)msg.textContent='';
    }catch(e){
      console.error('Cockpit session recovery',e);
      /* Une erreur de profil/catalogue ne doit jamais masquer une session valide. */
      if(auth)auth.classList.add('hidden');
      if(app)app.classList.remove('hidden');
      if(msg)msg.textContent='';
    }
  ...
  db.auth.getSession().then(({data,error})=>{
    ...
    if(data&&data.session)showLoggedIn(data.session);
    else if(logout)logout.classList.add('hidden');
  });
  ...
  /* Rend immédiatement visible toute erreur JS non capturée pendant le test. */
  window.addEventListener('error',e=>{
    console.error('Cockpit JS error',e.error||e.message);
  });
}
...
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{mount();stabilizeCockpitAuth()});else{mount();stabilizeCockpitAuth()}})();