(function(){'use strict';
/* WhatsAfrica: navigation + auth recovery + calls CTA.
   This file is loaded after the Cockpit inline code, so it deliberately
   re-wires critical controls and never lets a secondary data error hide a
   valid Supabase session. */
function cockpitNav(){
  if(!/Cockpit/i.test(document.title))return;
  const top=document.querySelector('.top .row');
  if(!top)return;
  if(document.getElementById('waCockpitNav'))return;
  const nav=document.createElement('div');
  nav.id='waCockpitNav';
  nav.className='row';
  nav.style.marginRight='4px';
  nav.innerHTML='<a href="/" class="btn ghost" style="text-decoration:none">🏠 Accueil</a><a href="/live.html" class="btn ghost" style="text-decoration:none">🔴 Business Live</a><a href="/crm.html" class="btn ghost" style="text-decoration:none">🎯 CRM</a><a href="/inbox.html" class="btn ghost" style="text-decoration:none">📥 Inbox</a>';
...
function stabilizeCockpitAuth(){
  ...
  const reveal=(session)=>{
    ...
    window.__WA_AUTH_USER=session.user;
    if(auth)auth.classList.add('hidden');
    if(app)app.classList.remove('hidden');
    if(logout)logout.classList.remove('hidden');
  };
  const recover=async(session)=>{
    if(!session||!session.user)return;
    reveal(session);
    try{window.__WA_AUTH_USER=session.user;if(typeof enter==='function')await enter(session.user)}catch(e){console.error('Cockpit secondary initialization',e)}
    reveal(session);
    if(msg)msg.textContent='';
  };
  db.auth.getSession().then(({data,error})=>{
    if(error){console.error('Cockpit getSession',error);return;}
    if(data&&data.session)recover(data.session);
  }).catch(e=>console.error('Cockpit session bootstrap',e));
  db.auth.onAuthStateChange((event,session)=>{
    if(session&&(event==='SIGNED_IN'||event==='TOKEN_REFRESHED'||event==='INITIAL_SESSION'))recover(session);
    if(event==='SIGNED_OUT'){
  ...
  if(login&&!login.dataset.waBound){
    ...
    login.addEventListener('click',async(e)=>{
      ...
      try{
        ...
        if(!r.data?.session||!r.data?.user)throw new Error('Session de connexion non reçue.');
        reveal(r.data.session);
        try{if(typeof enter==='function')await enter(r.data.user)}catch(e2){console.error('Cockpit login secondary init',e2)}
        reveal(r.data.session);
        if(msg)msg.textContent='';
      }catch(e){console.error('Cockpit login',e);if(msg)msg.innerHTML='<span class="err">'+String(e.message||'Connexion impossible.').replace(/[&<>]/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[m]))+'</span>'}
      finally{login.disabled=false;login.textContent='Se connecter'}
    },true);
  }
  if(logout&&!logout.dataset.waBound){
    logout.dataset.waBound='1';
    logout.addEventListener('click',async(e)=>{e.preventDefault();e.stopImmediatePropagation();try{await db.auth.signOut()}finally{location.reload()}},true);
  }
}
