...
create index if not exists ai_usage_events_user_period_idx
  on private.ai_usage_events(user_id, surface, created_at desc);

create index if not exists ai_usage_events_created_idx
  on private.ai_usage_events(created_at desc);
...
begin
  if p_user_id is null or p_surface not in ('smartlink_builder','smartlink_assistant') then
    raise exception 'AI_BUDGET_ARGUMENT_INVALID';
  end if;
  select coalesce(sum(estimated_cost_xof),0), count(*)
  ...
  return jsonb_build_object(
    'allowed', v_used < greatest(coalesce(p_budget_xof,350),0),
    'used_xof', round(v_used,2),
    'budget_xof', greatest(coalesce(p_budget_xof,350),0),
    'requests', v_requests,
...
create or replace function public.record_ai_usage(
  ...
  p_estimated_cost_xof numeric default 0,
...
begin
  ...
  insert into private.ai_usage_events(
    user_id,surface,model,provider,prompt_tokens,completion_tokens,
    estimated_cost_usd,estimated_cost_xof,status,metadata
  ) values (
    p_user_id,p_surface,left(p_model,160),left(p_provider,80),
    greatest(coalesce(p_prompt_tokens,0),0),greatest(coalesce(p_completion_tokens,0),0),
    greatest(coalesce(p_estimated_cost_usd,0),0),greatest(coalesce(p_estimated_cost_xof,0),0),
    case when p_status in ('success','fallback','blocked','error') then p_status else 'success' end,
...
revoke execute on function public.ai_budget_status(uuid,text,numeric) from anon, authenticated;
...
grant execute on function public.ai_budget_status(uuid,text,numeric) to service_role;
