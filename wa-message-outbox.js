...
let flushing=false,timer=null,probeTimer=null,probeBusy=false;
...
function networkError(e){const s=String(e?.message||e||'').toLowerCase();return !navigator.onLine||/failed to fetch|network|timeout|aborted|offline|load failed|err_internet|http_5|http_408/.test(s)}
...
async function probe(){if(probeBusy||navigator.onLine===false)return false;probeBusy=true;try{const c=new AbortController(),t=setTimeout(()=>c.abort(),4500);const r=await fetch(SB+'/rest/v1/?select=1&wa_probe='+Date.now(),{method:'GET',headers:{apikey:KEY},cache:'no-store',signal:c.signal});clearTimeout(t);return r.ok||r.status===401||r.status===403}catch{return false}finally{probeBusy=false}}
async function registerSync(){try{if(navigator.serviceWorker?.ready){const reg=await navigator.serviceWorker.ready;if(reg.sync?.register)await reg.sync.register('wassafrica-message-outbox')}}catch{}}
async function enqueue(payload){const item={...payload,queued_at:new Date().toISOString(),attempts:0};try{if(await probe()){try{await post(payload);note('Message envoyé');return {sent:true,queued:false}}catch(e){if(!networkError(e))throw e}}await put(item);note('En attente de connexion');registerSync();schedule();return {sent:false,queued:true}}catch(e){console.error('[WassAfrica] outbox enqueue',e);throw e}}
async function flush(){if(flushing||navigator.onLine===false)return;flushing=true;try{if(!(await probe()))return;const rows=(await all()).sort((a,b)=>new Date(a.queued_at)-new Date(b.queued_at));for(const row of rows){try{await post(row);await del(row.id);note('Message transmis');}catch(e){row.attempts=(row.attempts||0)+1;row.last_error=String(e?.message||e);await put(row);if(networkError(e)||e.status>=500||e.status===429)break;await del(row.id)}}}finally{flushing=false}}
async function schedule(){clearTimeout(timer);const pending=await all().catch(()=>[]);if(!pending.length)return;timer=setTimeout(()=>flush().catch(()=>{}),navigator.onLine?250:3000)}
async function connectivityCycle(){if(navigator.onLine===false){note('Hors connexion — les messages restent en attente');return}const pending=await all().catch(()=>[]);if(pending.length)await flush().catch(()=>{});if(pending.length)registerSync()}
window.WA_MESSAGE_OUTBOX={enqueue,flush,pending:async()=>all(),probe};
window.addEventListener('online',()=>{note('Connexion rétablie — vérification réseau…');connectivityCycle().catch(()=>{})},{passive:true});
...
document.addEventListener('visibilitychange',()=>{if(!document.hidden)connectivityCycle().catch(()=>{})});
probeTimer=setInterval(()=>connectivityCycle().catch(()=>{}),30000);
connectivityCycle().catch(()=>{});
})();