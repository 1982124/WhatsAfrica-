...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const payload = { model: models[0], models, messages: [{ role: "system", content: systemPrompt(business) }, ...history, { role: "user", content: message }], temperature: 0.1, max_tokens: 900, reasoning: { effort: "none" } };
    const headers = { "Content-Type": "application/json", Authorization: `Bearer ${process.env.OPENROUTER_API_KEY}`, "HTTP-Referer": "https://whatsafrica.vercel.app", "X-Title": "WhatsAfrica - Votre assistant commercial" };
    ...
    if (!response.ok) { console.error("OpenRouter error", response.status, data); return res.status(502).json({ error: "Le service de l'assistant est temporairement indisponible." }); }
    const answer = data?.choices?.[0]?.message?.content || data?.choices?.[0]?.text;
    if (!answer) { console.error("OpenRouter returned no assistant content", { model: data?.model, finish_reason: data?.choices?.[0]?.finish_reason, usage: data?.usage }); return res.status(502).json({ error: "L'assistant n'a pas pu générer de réponse. Réessayez." }); }
    return res.status(200).json({ answer: clean(answer, 6000), model: data.model || models[0] });
  } catch (error) {
    console.error("assistant", error);
    return res.status(500).json({ error: "L'assistant rencontre momentanément un problème. Réessayez dans quelques instants." });
  }
};
