...
function csv(value) {
  return String(value || '').split(',').map((x) => x.trim()).filter(Boolean).slice(0, 100);
}
...
async function validateAdminBearer(auth) {
  ...
  if (!SUPABASE_URL || !SUPABASE_PUBLISHABLE_KEY) return { ok: false, status: 503, reason: 'supabase_auth_env_missing' };
...
async function generateForCron({ hours, countries, zones, products }) {
  if (!SUPABASE_URL || !SUPABASE_SERVICE_ROLE_KEY) return { ok: false, status: 503, reason: 'supabase_cron_env_missing' };
  ...
  const filtered = observations.filter((item) =>
    matches(item.country_code, countries) && matches(item.location_text, zones) && matches(item.normalized_product, products)
  );
  ...
  for (const item of filtered) {
    ...
    group.count += 1;
    if (item.public_contact) group.contacts += 1;
  }
  const clusters = [...groups.values()].sort((a, b) => b.count - a.count || b.quantity - a.quantity);
  const reportPayload = {
    period_start: start.toISOString(),
    period_end: end.toISOString(),
    demand_count: filtered.length,
    grouped_product_count: clusters.length,
    total_quantity: { value: clusters.reduce((sum, item) => sum + item.quantity, 0) },
    top_demands: clusters,
    source_breakdown: filtered.reduce((acc, item) => { const key = item.source_platform || 'unknown'; acc[key] = (acc[key] || 0) + 1; return acc; }, {}),
    contactable_count: filtered.filter((item) => item.public_contact).length,
    unresolved_count: filtered.filter((item) => ['detected', 'verified', 'clustered', 'contact_ready'].includes(item.status)).length,
    report: { generated_by: 'wassafrica-demand-intelligence', hours, scope: { countries, zones, products }, clusters },
  ...
  if (saved?.id && clusters.length) {
    await fetch(`${SUPABASE_URL}/rest/v1/demand_report_items`, {
      method: 'POST',
      headers: { ...headers, Prefer: 'return=minimal' },
      body: JSON.stringify(clusters.map((cluster) => ({
        report_id: saved.id,
  ...
  return {
    ...
    hours,
    scope: { countries, zones, products },
    demand_count: filtered.length,
    cluster_count: clusters.length,
    total_quantity: clusters.reduce((sum, item) => sum + item.quantity, 0),
    clusters
  };
}
...
export default async function handler(req, res) {
  ...
  if (!SUPABASE_URL || !SUPABASE_PUBLISHABLE_KEY) return res.status(503).json({ ok: false, error: 'supabase_auth_env_missing' });
  ...
  const countries = normalizeArray(csv(req.query?.countries || process.env.DEMAND_COUNTRIES));
  const zones = normalizeArray(csv(req.query?.zones || process.env.DEMAND_ZONES));
  const products = normalizeArray(csv(req.query?.products || req.query?.product || process.env.DEMAND_PRODUCTS));
  const auth = req.headers.authorization || '';
  const cronAuthorized = Boolean(process.env.CRON_SECRET && auth === `Bearer ${process.env.CRON_SECRET}`);

  let result;
  if (cronAuthorized) {
    result = await generateForCron({ hours, countries, zones, products }).catch((error) => ({ ok: false, status: 500, reason: error?.message || 'cron_generation_failed' }));
  } else {
  ...
  const scopeLabel = [...(countries || []), ...(zones || [])].join(', ') || 'Monde entier';
  ...
  const text = [
    'WASSAFRICA — RAPPORT DES BESOINS',
    `Période: ${result.window_start} → ${result.window_end}`,
    `Durée: ${result.hours} h`,
    `Zone(s): ${scopeLabel}`,
    `Produit(s): ${productLabel}`,
    `Demandes: ${result.demand_count}`,
    `Produits regroupés: ${result.cluster_count}`,
    '',
    ...(lines.length ? lines : ['Aucune demande détectée sur ce périmètre.']),
    '',
    `Rapport ID: ${result.report_id || 'n/a'}`
  ].join('\n');
