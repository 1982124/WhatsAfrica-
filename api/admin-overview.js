export default async function handler(req, res) {
  ...
  if (!url || !serviceKey) return res.status(503).json({ error: 'admin_service_not_configured' });
  ...
  const common = { apikey: serviceKey, Authorization: `Bearer ${serviceKey}` };
  try {
    ...
    const admin = await fetch(`${url}/rest/v1/platform_admins?user_id=eq.${encodeURIComponent(user.id)}&select=user_id&limit=1`, {
    ...
    const users = await fetch(`${url}/rest/v1/profiles?select=id&limit=1`, {
      method: 'HEAD', headers: { ...common, Prefer: 'count=exact' }, signal: AbortSignal.timeout(3500)
    });
