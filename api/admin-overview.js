export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'POST') {
      ...
      const emailResponse = await fetch('https://api.resend.com/emails', { method: 'POST', headers: { Authorization: `Bearer ${resendKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ from, to: [user.email], subject: 'Votre code de sécurité WhatsAfrica', text: `Votre code de sécurité administrateur WhatsAfrica est : ${code}\n\nCe code expire dans 10 minutes et ne peut être utilisé qu'une seule fois.`, html: `<div style="font-family:Arial,sans-serif;max-width:520px;margin:auto"><h2>WhatsAfrica — Sécurité administrateur</h2><p>Votre code de sécurité est :</p><div style="font-size:32px;font-weight:800;letter-spacing:8px;text-align:center;padding:18px;border:1px solid #ddd;border-radius:12px">${code}</div><p>Ce code expire dans <b>10 minutes</b> et ne peut être utilisé qu'une seule fois.</p></div>` }), signal: AbortSignal.timeout(10000) });
